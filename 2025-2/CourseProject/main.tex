...
% Título
% =====================
\title{\textbf{TP — Aprendizagem Contínua em Duas Partes}\\\large Disciplina de Otimização Não Linear}
...
% ======================================================
\section*{Visão geral}
Ao treinar sequencialmente em A e depois em B, modelos tendem a esquecer A. Estudaremos o \emph{trade-off} estabilidade–plasticidade em duas etapas complementares:
\begin{itemize}
  \item \textbf{Parte 1} (minimização): penalização fixa de deriva para controlar o esquecimento.
  \item \textbf{Parte 2} (min–max): penalização adaptativa via jogo entre parâmetros e um multiplicador~$\lambda$.
\end{itemize}

\subsection*{Dados e modelo}
\textbf{Split--MNIST}: A = \{0–4\}, B = \{5–9\}.  
Modelo sugerido: MLP simples ou CNN pequena. Perda: cross-entropy.

\subsection*{Notação}
Parâmetros atuais $\bm{\theta}$; solução após treinar A: $\bm{\theta}_A$.  
Medida de deriva:
\[
...
Após treinar A e congelar $\bm{\theta}_A$, otimize na Tarefa B:
\[
...
\begin{enumerate}
  ...
  \item \textbf{Treino em B} para cada $\lambda$ da grade, usando o mesmo otimizador e esquema de treino.
...
\begin{itemize}
  ...
  \item \textbf{Fronteira Pareto-like}: destaque dos pontos não dominados.
\end{itemize}
...
Explique a formulação min–max e por que min e max competem; compare OGDA vs. Extragradient visualmente e textualmente; relacione com os resultados da Parte~1.
